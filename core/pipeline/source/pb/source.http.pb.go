...
func RegisterSourceServiceHandler(r http.Router, srv SourceServiceHandler, opts ...http.HandleOption) {
	...
	add_DeleteByRemote := func(method, path string, fn func(context.Context, *PipelineSourceDeleteByRemoteRequest) (*PipelineSourceDeleteResponse, error)) {
		handler := func(ctx context.Context, req interface{}) (interface{}, error) {
			return fn(ctx, req.(*PipelineSourceDeleteByRemoteRequest))
		}
		var DeleteByRemote_info transport.ServiceInfo
		if h.Interceptor != nil {
			DeleteByRemote_info = transport.NewServiceInfo("erda.core.pipeline.source.SourceService", "DeleteByRemote", srv)
			handler = h.Interceptor(handler)
		}
		r.Add(method, path, encodeFunc(
			func(w http1.ResponseWriter, r *http1.Request) (interface{}, error) {
				...
				r = r.WithContext(ctx)
				var in PipelineSourceDeleteByRemoteRequest
				if err := h.Decode(r, &in); err != nil {
					return nil, err
				}
				var input interface{} = &in
	...
	add_DeleteByRemote("DELETE", "/api/pipeline-sources/actions/delete-by-remote", srv.DeleteByRemote)
}
