...
var _ urlenc.URLValuesUnmarshaler = (*PipelineSourceDeleteByRemoteRequest)(nil)
...
// PipelineSourceDeleteByRemoteRequest implement urlenc.URLValuesUnmarshaler.
func (m *PipelineSourceDeleteByRemoteRequest) UnmarshalURLValues(prefix string, values url.Values) error {
	for key, vals := range values {
		if len(vals) > 0 {
			switch prefix + key {
			case "remote":
